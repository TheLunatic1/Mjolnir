...
#[async_trait]
impl ProtocolClient for Http1Client {
    async fn execute(&self, req: &RequestSpec) -> RequestSample {
        ...
        if let Some(auth) = &req.auth {
            match auth.auth_type.as_str() {
                ...
                "api_key" => {
                    if let (Some(k), Some(v)) = (&auth.api_key_header, &auth.api_key_value) {
        ...
        match resp {
            Ok(r) => {
                ...
                RequestSample {
                    timestamp,
                    duration_us,
                    ttfb_us: duration_us / 2, // Approximate for high-throughput mode unless traced
                    dns_us: 100,
                    tcp_us: 200,
                    tls_us: 300,
                    bytes_in,
