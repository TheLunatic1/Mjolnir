...
import { Users, Zap, Clock, AlertTriangle, ArrowUpRight, CheckCircle2, XCircle, Activity } from 'lucide-react';

export const DashboardPage: React.FC = () => {
  const { liveMetrics, metricsHistory, activeScenario, engineStatus } = useStore();

  // Safe accessors with optional chaining — prevents crash when latencies are missing
  const labels = metricsHistory.map((f) => `${Math.round(f.elapsed_seconds ?? 0)}s`);

  const rpsData = metricsHistory.map((f) => Math.round(f.current_rps ?? 0));
  const vusData = metricsHistory.map((f) => f.current_vus ?? 0);

  const p50Data = metricsHistory.map((f) => Number((f.latencies?.total_duration?.p50 ?? 0).toFixed(1)));
  const p95Data = metricsHistory.map((f) => Number((f.latencies?.total_duration?.p95 ?? 0).toFixed(1)));
  const p99Data = metricsHistory.map((f) => Number((f.latencies?.total_duration?.p99 ?? 0).toFixed(1)));

  const bwInData = metricsHistory.map((f) =>
    Number(((f.bandwidth_in_bytes_per_sec ?? 0) / 1024).toFixed(1))
  );
  const bwOutData = metricsHistory.map((f) =>
    Number(((f.bandwidth_out_bytes_per_sec ?? 0) / 1024).toFixed(1))
  );

  const hasData = metricsHistory.length > 0 || liveMetrics !== null;
  const isEngineReady = engineStatus === 'idle' || engineStatus === 'running';
      ...
      <div className="flex items-center justify-between glass-panel p-5 rounded-xl border border-slate-800 bg-gradient-to-r from-slate-900/90 via-panel to-primary-950/20">
        ...
        <div className="text-right font-mono">
          ...
          <div className="text-3xl font-black text-cyan-400 animate-pulse">
            {(liveMetrics?.total_requests ?? 0).toLocaleString()}
          </div>
        </div>
      </div>

      {/* Empty State — when engine is not running and no data */}
      {!hasData && (
        <div className="glass-card p-12 rounded-xl border border-slate-800 flex flex-col items-center justify-center text-center space-y-4">
          <div className="w-16 h-16 rounded-2xl bg-slate-800/80 border border-slate-700 flex items-center justify-center">
            <Activity className="w-8 h-8 text-slate-500" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-slate-300 font-['Outfit',sans-serif] mb-1">
              No Test Data Yet
            </h3>
            <p className="text-sm text-slate-500 max-w-sm">
              {!isEngineReady
                ? 'Start the Mjolnir Native Engine from the top toolbar, then click "Launch Strike" to begin a load test.'
                : 'Engine is ready. Configure your scenario in the Visual Builder and click "Launch Strike".'}
            </p>
          </div>
          {isEngineReady && (
            <div className="flex items-center gap-1.5 text-xs text-emerald-400 font-mono bg-emerald-400/10 border border-emerald-500/30 px-3 py-1.5 rounded-full">
              <div className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
              Engine Ready — Awaiting Strike Command
            </div>
          )}
        </div>
      )}
